  ...
  if (!isLoading && !isError && data) {
    ...
    if (ra?.configured) {
      if (ra.reachable) {
        ...
        if (tone.includes('amber') && !w?.configured) tone = 'border-slate-300 bg-slate-100 text-slate-900';
        ...
        if (tone.includes('slate-100')) tone = 'border-amber-200 bg-amber-50 text-amber-900';
      }
    }
    ...
    <div className={`border-b px-3 py-2 text-xs sm:px-6 sm:text-sm lg:px-10 ${tone}`}>
      <div className="mx-auto flex max-w-none flex-wrap items-center justify-between gap-2">
        ...
        {isAdmin && !onSettingsPage ? (
          <Link
            to="/system-settings"
            className="shrink-0 rounded-lg bg-zinc-950 px-3 py-2 text-xs font-semibold text-white hover:bg-black min-h-[40px] min-w-[44px] inline-flex items-center justify-center sm:min-h-0 sm:py-1.5"
          >
            Settings
          </Link>
        ) : null}
